...
Future<void> main() async {
  final ip = "172.20.10.4";
  final server = await ServerSocket.bind(ip, 3000);
  print("Server is running on ${ip}:3000");
    ...
    (Uint8List data) async {
      ...
      if (message.startsWith("join;")) {
        ...
        if(players.length %2 == 0){
          players[players.length-1].enemy = players[players.length-2].username;
          players[players.length-2].enemy = players[players.length-1].username;

          await sendMessageToPlayer(players[players.length-1].username, "enemy;${players[players.length-1].enemy};end;");
          await sendMessageToPlayer(players[players.length-2].username, "enemy;${players[players.length-2].enemy};end;");
        }
        ...
        try{
          ...
          for (int i = 0; i < players.length; i++) {
            if (players[i].username == message.split(";")[1]) {
              players[i].placeShip(int.parse(message.split(";")[2]), int.parse(message.split(";")[3]), int.parse(message.split(";")[4]), message.split(";")[5]);
              camp = players[i].printGrid();
            }
          }
            ...
            for (int e = 0; e < players.length; e++) {
              if (players[e].username == players[i].enemy) {
                if(players[e].isReady){
                  ...
                  sendMessageToPlayer(players[e].username, "turn;${players[e].printEnemyGrid()};end;");
                } else{
                  sendMessageToPlayer(message.split(";")[1], "player ${players[i].enemy} is not ready");
                }
              }
            }
          ...
          for (int i = 0; i < players.length; i++) {
            if (players[i].username == message.split(";")[1]) {
              ...
              for (int e = 0; e < players.length; e++) {
                  if (players[e].username == players[i].enemy) {
                    en = e;
                    players[i].updateTable(int.parse(message.split(";")[2]), int.parse(message.split(";")[3]), players[e].shoot(int.parse(message.split(";")[2]), int.parse(message.split(";")[3])));
                  }
              }
        ...
        if(players[en].victory()){
          ...
          sendMessageToPlayer(players[en].username, "turn;${players[en].printEnemyGrid()};end;"); 
          sendMessageToPlayer(message.split(";")[1], "table;$camp;end;");
        }
